...
### 2. legacy 格式的 COMPONENT 识别（原列表 ① 的剩余部分）
- **盲区**：`decodeNodeType` 的 legacy `1c 07` 分支只有 `06`=INSTANCE、`03/09/0a`=FRAME、`01 00 09/0a`=GROUP、`01 00 02`=BOOLEAN_OPERATION，**没有 COMPONENT**；带子节点的组件会被归为 `FRAME`。火车票页面真值恰好不含 COMPONENT，所以「828/829、0 错判」并未覆盖这里。
- **首选验证（不需要真值）**：`hasSelfUkey`（几何段含 `+<selfId>\0`）在 modern 下是 COMPONENT 的精确判据、零假阳性，而 ukey 机制与容器编码无关。**统计 legacy 火车票里「几何段含自引用 ukey」的节点当前被判成什么**——若大量落在 `FRAME`，即命中盲区。
